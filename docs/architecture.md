...
VPC Flow Logs are enabled to capture network traffic metadata for auditing and security monitoring. All logs are encrypted using AWS KMS to ensure compliance and data protection.

This architecture ensures strong security boundaries, least-privilege access, high availability, and clear separation between public-facing and internal components, while supporting controlled deployments and manual database migrations.
